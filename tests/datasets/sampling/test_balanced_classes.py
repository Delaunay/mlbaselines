...
def _dummy_indices_method(rng, indices, n_train, n_valid, n_test):
    return dict(train=indices[:n_train], valid=indices[n_train:n_train + n_valid],
...
def test_balanced_random_balance(classes, N_TRAIN, N_VALID, N_TEST, N_POINTS, N_CLASSES):
    indices = balanced_random_indices(_dummy_indices_method, classes, N_POINTS, seed=1)
...
def test_balanced_random_too_many_points(classes, N_POINTS, N_CLASSES):
    with pytest.raises(AssertionError) as exc:
        balanced_random_indices(_dummy_indices_method, classes, N_POINTS + N_CLASSES, seed=1)
