...
from .loss_scaling import LossScaler
from .utils import master_params_to_model_params, clip_grad_norm
...
class FP16Optimizer(object):
            ...
            for i, param in enumerate(param_group['params']):
                if param.requires_grad:
                    if param.type() == 'torch.cuda.HalfTensor' or param.type() == 'torch.HalfTensor':
                        self.maybe_print(f"FP16_Optimizer received torch.cuda.HalfTensor with {param.size()}")
                        ...
                        self.maybe_print(f"FP16_Optimizer received torch.cuda.FloatTensor with {param.size()}")
    ...
    def step(self, closure=None):  # could add clip option.
        ...
        if self.overflow:
            print(f"Gradient overflow.  Skipping step, reducing loss scale to {self.loss_scaler.loss_scale()}")
            return
